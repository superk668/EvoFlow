...
- 系统必须在用户登录成功后，将“已登录状态”持久化保存，以支持页面刷新、重新打开页面后仍保持登录态。
- 推荐保存介质与最小字段（前端实现约定）：
  - `localStorage` 键名：`evoflow_auth`
  - 值结构（JSON）：`isLoggedIn`、`loginAt`、`userDisplayName`、`phoneNumber`（脱敏或原值由实现决定）、`token`（如有）
  - 需要登录的页面触发跳转到登录页时，应在 `sessionStorage` 记录 `postLoginRedirect`（原始目标 Path 路由）
- 退出登录时必须清理登录态（包括 `evoflow_auth` 与与用户相关的会话缓存），并将页面回到未登录可访问的入口页。
- 任何需要登录才能访问的页面，若检测到未登录，应跳转到 `/login`，并在登录成功后回跳到 `postLoginRedirect`（若存在）。
...
**Scenario: 1.1.3 用户使用有效的账号和密码成功登录**
    ...
    When 用户点击“登录”按钮
    Then 系统验证账号和密码匹配
    And 系统保存“已登录”会话状态（见“0. 会话状态保存约定”）
    And 页面跳转至携程首页或登录前页面（若存在 `postLoginRedirect`）
...
**Scenario: 1.4.1 登录后刷新页面仍为登录态**
    Given 用户已成功登录并保存“已登录”会话状态
    When 用户刷新页面或重新打开站点首页
    Then 系统应从持久化存储恢复登录态
    And 首页顶部栏展示用户名入口，不展示“登录/注册”按钮

**Scenario: 1.4.2 未登录访问受限页面应先跳转登录并回跳**
    Given 用户未登录
    And 用户访问一个需要登录的页面
    When 系统检测到未登录
    Then 系统跳转至登录页 `/login`
    And 系统记录原始目标路由为 `postLoginRedirect`
    When 用户登录成功
    Then 系统回跳至 `postLoginRedirect` 指向的页面

**Scenario: 1.4.3 用户退出登录后清理会话状态**
    Given 用户已登录
    When 用户点击“退出登录”
    Then 系统清理“已登录”会话状态与用户缓存
    And 页面切换为未登录态展示（显示“登录/注册”入口）
...
**Scenario: 2.1.6a 用户未勾选服务协议点击下一步**
    Given 用户已填写完手机号和验证码
    And 用户输入了正确的验证码
    And 用户未勾选“服务协议”复选框
    When 用户点击“下一步，设置密码”按钮
