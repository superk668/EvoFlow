...
## 0. 数据一致性与持久化总则（用于避免“保存失败/信息不一致/展示不完整”）

### 0.1 单一数据源与跨页一致性
- 同一业务对象在不同页面（列表/详情/编辑/支付/完成）必须来源于同一份持久化数据，禁止页面内硬编码静态占位覆盖真实数据。
- “页面展示值”必须可回溯到“持久化数据对象”的具体字段；字段名、层级与脱敏规则必须在本需求文档中明确。

### 0.2 保存成功的判定与兜底
- 任何“保存/新增/取消订单”等写操作，必须先满足本地持久化成功（写入 localStorage / sessionStorage 成功）才允许提示“成功”并切换页面状态。
- 若存在后端接口同步：
  - 后端同步失败不得回滚已成功写入的本地数据；应提示“已保存，本次同步失败，请稍后重试”并允许用户继续使用本地数据。
  - 仅当本地持久化失败时，提示“保存失败/新增失败/取消失败”并保留用户输入。

### 0.3 关键存储约定（键名必须一致）
- 个人信息：`localStorage` 键名 `evoflow_user_profile`
- 常用旅客：`localStorage` 键名 `evoflow_common_travelers`
- 订单中心：`localStorage` 键名 `evoflow_orders`
- 登录回跳：`sessionStorage` 键名 `postLoginRedirect`（见登录注册需求文档）

### 0.4 脱敏展示规则（避免同一信息在不同页“看起来不一致”）
- 手机号：保留前三位与后四位，中间以 `*` 替换（示例：`138*****3769` 或带国家码 `86-138*****3769`）
- 证件号（身份证）：保留前三位与后两位，中间以 `*` 替换（示例：`360***********12`）
...
### 1.8 常用旅客信息：数据结构与持久化（用于避免“新增/编辑后不生效、刷新丢失、展示不一致”）

**1.8.1 存储约定**
- 存储位置：`localStorage`
- 键名：`evoflow_common_travelers`
- 数据结构：JSON 数组

**1.8.2 旅客数据结构（最低字段 + 可选扩展）**
- 最低字段（必须具备，缺失视为脏数据并在列表中隐藏该条或展示“数据异常”）：
  - `travelerId`：字符串，旅客唯一标识（新增时生成）
  - `isSelf`：布尔值，是否“本人”
  - `nameZh`：中文名（与英文名二选一，允许为空字符串但不能两者都为空）
  - `lastName`：英文姓（可为空）
  - `firstName`：英文名（可为空）
  - `birthday`：生日（可为空；非空时格式 `yyyy-MM-dd`）
- 推荐扩展字段（当前版本可不实现，但展示逻辑必须容错为 `—` 占位）：
  - `gender`、`nationality`、`birthPlace`
  - `phoneNumber`
  - `idType`、`idNumber`、`idNumberMasked`
  - `createdAt`、`updatedAt`

**1.8.3 约束规则**
- 本人唯一：任意时刻最多只能存在 1 条 `isSelf=true` 的记录。
- 本人不可删除：`isSelf=true` 的记录在任何删除入口均不可删除（列表/批量/详情）。
- 证件唯一：当存在 `idType` 与 `idNumber` 时，`idType + idNumber` 在旅客集合内必须唯一（新增与编辑均校验）。
- 展示脱敏：列表与详情展示证件号时，必须使用脱敏值（如 `idNumberMasked`），不得直接展示 `idNumber` 原文。

#### Scenario: 新增旅客后必须持久化并可回读（正常流程）
    Given 用户在新增页填写合法信息并点击“保存”
    When 系统写入 `localStorage.evoflow_common_travelers` 成功
    Then 提示“新增成功”并返回列表页
    And 列表页展示新增的旅客条目
    And 刷新页面后该条目仍存在

#### Scenario: 设置为本人时自动保证唯一（状态一致性）
    Given 列表中已存在一条 `isSelf=true` 的旅客
    When 用户在新增或编辑时勾选“设置为本人”并保存成功
    Then 该新/编辑后的旅客记录为 `isSelf=true`
    And 其他所有旅客记录必须自动变更为 `isSelf=false`

#### Scenario: 本地写入失败时不允许提示成功（系统异常）
    Given 用户点击“保存”
    When `localStorage` 写入失败或抛出异常
    Then 提示“系统繁忙，请稍后重试”
    And 保留用户输入用于再次提交
...
#### Scenario: 系统异常：接口失败但本地保存成功（兜底）
    Given 用户在编辑态点击“保存”
    And 本地持久化写入成功
    When 服务器返回错误或网络超时
    Then 页面回到只读态并展示更新后的字段值
    And 提示“已保存，本次同步失败，请稍后重试”
...
### 3.4.1 持久化与接口契约（用于避免“更改个人信息无法保存”）

- 本地持久化：
  - 存储位置：`localStorage`
  - 键名：`evoflow_user_profile`
  - 写入时机：点击“保存”且前端校验通过后，必须先写入本地持久化；本地写入失败则判定保存失败。
- 后端同步（如接入）：
  - `POST /api/user-center/my-info`
  - 请求体字段：`nickname`、`name`、`gender`、`birthday`（`yyyy-MM-dd`，可为空）
  - 成功条件：HTTP 2xx；失败条件：HTTP >= 400 或网络异常
  - 版本冲突：HTTP 409，表示资料已被其他端更新，前端提示“信息已被更新，请刷新后重试”，且不覆盖本地最新数据
  - 同步失败提示：本地已保存时，提示“已保存，本次同步失败，请稍后重试”，不得回滚本地数据
...
#### Scenario: 订单条目信息完整展示（防止“订单条目显示的信息不完整”）
    Given 用户已登录且订单存储中存在一条机票订单
    When 订单管理列表页渲染完成
    Then 每条订单卡片至少展示：订单号、下单时间、订单状态、行程标题（出发城市→到达城市）、出发日期、出行人、订单总金额
    And 若订单状态为 `pending_payment`，则展示“去支付”入口
    And 若订单状态为 `pending_payment` 或 “未出行”，则展示“取消订单”入口
...
#### Scenario: 详情展示字段完整且与列表一致（防止“订单条目显示的信息不完整”）
    Given 用户从订单列表页点击进入某订单详情页
    When 详情页渲染完成
    Then 详情页展示的路线、乘机人、联系人、价格明细必须来自该订单的 `details` 快照数据
    And 详情页展示的乘机人姓名与证件号（脱敏）必须与订单列表/支付页落盘时一致
    And 若 `details` 或其子字段缺失，则对应区域展示 `—` 占位且页面不崩溃
...
> 正常流程，用户完成取消操作。
    ...
    Then 系统应在本地共享订单存储中将该订单状态更新为 `canceled`
    And 前端弹出“订单取消成功”提示
    And 页面刷新，状态更新为“已取消”
...
- 本地共享订单存储约定：
  ...
  - `details` 最低字段（用于列表与详情展示完整信息）：
    - `route`：`fromCity`、`toCity`
    - `passenger`：`name`、`idType`、`idNumberMasked`
    - `contact`：`phoneNumber`
    - `priceItems`：数组（至少包含合计口径所需的项目名、单价、数量）
  - 字段缺失容错：若历史数据缺少 `details` 或其子字段，页面必须展示明确占位（如 `—`），不得出现空白布局或报错；同时不影响“返回/重新下单”等与该字段无关的操作
