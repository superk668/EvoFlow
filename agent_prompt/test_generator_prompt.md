...
4.23.  **生成目标功能测试：**
    ...
    * **网络异常测试 (Network Failure):** 必须为关键提交动作（如“登录”“完成注册”“下单”）生成至少 1 条网络异常用例：Mock `fetch` 抛出异常/Promise reject，断言页面显示可读的错误提示，并且 Loading/禁用态能在失败后被解除（按钮恢复可点击，允许重试）。
    ...
    * **禁止测试后门 (No Test Backdoor):** 严禁要求开发者在生产逻辑中硬编码 magic 常量来让测试通过。若需要 token/验证码，应在测试中通过调用上游接口获取并传递。
